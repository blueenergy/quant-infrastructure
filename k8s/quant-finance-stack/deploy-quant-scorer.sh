#!/usr/bin/env bash
# Render and deploy the singleton daily quant-scorer scheduler.
...
MANIFEST="$SCRIPT_DIR/base/quant-scorer.yaml"
...
scorer_runtime="${QUANT_SCORER_RUNTIME:-}"
if [ -z "$scorer_runtime" ]; then
  scorer_runtime="$(env_file_get QUANT_SCORER_RUNTIME "$DEPLOY_ENV_FILE")"
fi
...
if [ "$scorer_runtime" != "external_k8s" ]; then
  echo "ERROR: QUANT_SCORER_RUNTIME must be external_k8s before deploying the K8s scheduler" >&2
  exit 1
fi

echo "Deploying singleton quant-scorer image $image to namespace $NAMESPACE"
render_manifest | kubectl -n "$NAMESPACE" apply -f -
kubectl -n "$NAMESPACE" rollout status deployment/quant-scorer --timeout="$ROLLOUT_TIMEOUT"
