...
> 本目录为基础设施栈，与 `../apps/`（业务应用栈）并列。所有 `docker compose`
> 命令均在**本目录（`infra/`）下执行**。Compose 项目名通过 `docker-compose.yml`
> 里的 `name: quant-infrastructure` 显式钉死，命名卷为 `quant-infrastructure_*`。
...
业务服务（API、Analyzer、前端等）的 Kustomize 清单见 **[../k8s/quant-finance-stack/README.md](../k8s/quant-finance-stack/README.md)**（⚠️ **草稿，尚未真正启用**）。生产目前使用本 `infra/` + `../apps/` 两套 Docker Compose；k8s 清单仅供后续迁移参考。
...
```
infra/                          # 本目录（基础设施栈）
├── docker-compose.yml          # 编排文件（name: quant-infrastructure）
├── .env.example                # 环境变量模板
├── .env                        # 环境变量（需自己创建，已 gitignore）
├── mongodb/
...
│   ├── dashboards/
│   └── datasources/
├── hermes-data/                # Hermes 运行时数据（容器创建，已 gitignore）
└── scripts/
    ├── deploy.sh               # 部署脚本
    ├── monitor.sh              # 监控脚本
...
# 调整内存限制（编辑本目录的 docker-compose.yml）
